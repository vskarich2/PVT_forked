...
class FrustumPointNetLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        device = torch.device(device=center.device)
        batch_id = torch.arange(batch_size).to(device)
        ...
        center_loss = huber_loss(torch.norm(center_target - center, dim=-1), delta=2.0)
        center_reg_loss = huber_loss(torch.norm(center_target - center_reg, dim=-1), delta=1.0)
        ...
        size_residual_normalized_target = size_residual_target / self.size_templates[size_template_id_target].to(device)
        ...
        heading = (heading_residuals[batch_id, heading_bin_id_target]
                   + self.heading_angle_bin_centers[heading_bin_id_target].to(device)) # (B, )
        ...
        size = (size_residuals[batch_id, size_template_id_target]
                + self.size_templates[size_template_id_target].to(device))  # (B, 3)
        corners = get_box_corners_3d(centers=center, headings=heading, sizes=size, with_flip=False)  # (B, 3, 8)
        heading_target = self.heading_angle_bin_centers[heading_bin_id_target].to(device) + heading_residual_target  # (B, )
        size_target = self.size_templates[size_template_id_target].to(device) + size_residual_target  # (B, 3)
